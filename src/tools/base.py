...
from tools._function_type_hints_utils import (
    TypeHintParsingException,
    _convert_type_hints_to_json_schema,
    get_imports,
    get_json_schema,
)
from tools.agent_types import handle_agent_input_types, handle_agent_output_types
from tools.tool_validation import MethodChecker, validate_tool_attributes
from tools.utils import BASE_BUILTIN_MODULES, _is_package_available, get_source, instance_to_source
...
class ToolMemory:
    """Memory storage for tools to track execution times and samples across invocations."""
    
    def __init__(self):
        self.execution_times: List[float] = []
        self.samples: List[Any] = []
        self._estimated_time: Optional[float] = None
    
    def add_execution_time(self, execution_time: float) -> None:
        """Add an execution time to the memory."""
        if execution_time >= 0:  # Only store non-negative times
            self.execution_times.append(execution_time)
    
    def add_sample(self, sample: Any) -> None:
        """Add a sample result to the memory."""
        self.samples.append(sample)
    
    def get_estimated_time(self) -> Optional[float]:
        """Get the current estimated execution time."""
        if self._estimated_time is not None:
            return self._estimated_time
        
        # If no explicit estimate, calculate from historical data
    ...
    def get_latest_samples(self, n: int = 5) -> List[Any]:
        """Get the latest N samples."""
        return self.samples[-n:] if self.samples else []
...
class ToolCollection:
    ...
    @classmethod
    @contextmanager
    def from_mcp(cls, server_parameters) -> Generator["ToolCollection", None, None]:
...
class BaseArithmeticTool(Tool):
    """Base class for simple arithmetic operations.

    Subclasses must define:
        - name
        - description
        - operation (callable via _op method or overridden forward)
    """

    # Common metadata (subclasses override name/description)
    inputs: Dict[str, Dict[str, str]] = {
        "a": {"type": "number", "description": "First operand."},
        "b": {"type": "number", "description": "Second operand."},
    }
    output_type = "string"
    # Use generic signature (skip validation) to stay flexible with inheritance
    skip_forward_signature_validation = True

    def forward(self, *args, **kwargs) -> str:  # accept generic, extract a,b
        if len(args) >= 2:
            a, b = args[0], args[1]
        else:
            a = kwargs.get("a")
        ...
        if a is None or b is None:
            raise ValueError("Expected two operands 'a' and 'b'.")
        result = self._compute(float(a), float(b))
        self.memory.add_sample({"a": a, "b": b, "result": result, "tool": self.name})
        return str(result)

    # Implementation hook
    def _compute(self, a: float, b: float) -> float:  # pragma: no cover - overridden in subclasses
